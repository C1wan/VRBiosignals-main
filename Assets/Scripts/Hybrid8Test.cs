...
public class Hybrid8Test : MonoBehaviour
{
    ...
    public void OnDataReceived(int nSeq, int[] data)
    {
        ...
        if (nSeq % subsamplingFactor == 0)
        {
            ...
            {
                //  Debug.Debug.Log(data[j]);
                outputString += data[j] + "\t";
            }
            UnityEngine.Debug.Log(outputString);
            // Show the values in the GUI.
            /* OutputMsgText.text = outputString;
             if (outputString == "" || outputString == null || String.IsNullOrEmpty(outputString)){
                 outputString = "1";
             }
             */
            // Giving the values of each channel to a chart
            valueCH1 = data[0];
            valueCH2 = data[1];
            valueCH3 = data[2];
            valueCH4 = data[3];
